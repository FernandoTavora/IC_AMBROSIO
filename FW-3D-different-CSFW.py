...
# Mesmo processo feito nos outros códigos
    ...
    # Malha amostrada
    nx_vis, nz_vis = 200, 500  # Pode reduzir a resolução para testes mais rapidos
    ...
    # Folhas de luz
    for plano_y in folhas_y:  # Plano de observação atual
        Ex_plano = np.zeros_like(Z_grid, dtype=complex)
        Ez_plano = np.zeros_like(Z_grid, dtype=complex)

        for idx_yp, yp in enumerate(folhas_y):
            # Seleciona a imagem correta projetada por esta folha específica
            f_matrix_atual = lista_f_matrices[idx_yp]
    ...
    for i, y_plane in enumerate(folhas_y):
        ...
        ax.plot_surface(
            ...
            facecolors=cores,
            rstride=1, cstride=1,  # Mude para 3 ou 5 para testes rápidos
    ...
    # Limites da câmera atualizados para enquadrar N folhas perfeitamente
    ax.set_xlim([0, L * 1e6])
