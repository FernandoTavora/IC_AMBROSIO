...
def simular_cvfw_3D_multiplas(lista_img_paths):
    ...
    for plano_y in folhas_y:  # Plano de observação da câmera
    ...
    for i, y_plane in enumerate(folhas_y):
        ...
        alpha_channel = np.clip((i_norm - threshold) / (1 - threshold), 0, 1)
    ...
    # Rótulos acompanhando a nova ordem para ficarem idênticos ao "caso ideal"
    ax.set_xlabel(r'$z$ ($\mu m$)')
    ax.set_ylabel(r'$x$ ($\mu m$)')
    ax.set_zlabel(r'$y$ ($\mu m$)')

    # Limites da câmera atualizados para a nova ordem
    ax.set_xlim([0, L * 1e6])
...
# Lista de imagens com padrões diferentes
minhas_imagens = [
    "fatia_1.png",
    "fatia_2.png",
    "fatia_3.png",
